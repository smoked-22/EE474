...
def conv_layer(input, conv_kernel, conv_strides, conv_padding='SAME', name='Conv'):
    with tf.name_scope(name):
        w = tf.Variable(tf.truncated_normal(conv_kernel, mean=.0, stddev=.1), name='W')
...
def pool_layer(input, poo_kernel, pool_strides, poo_padding='VALID', name='Pool'):
    with tf.name_scope(name):
        pool = tf.nn.max_pool(input, ksize=poo_kernel, strides=pool_strides, padding=poo_padding)
        return pool


def fc_layer(input, size_in, size_out, dropoutProb=None, name='FC'):
    with tf.name_scope(name):
        w = tf.Variable(tf.truncated_normal([size_in, size_out], stddev=.1), name='W')
...
def CNN_model(x, n_classes, nShape, nChannels, dropout):
    ...
    flattened = tf.reshape(pool2, [-1, pool2_shape[1] * pool2_shape[2] * pool2_shape[3]])
...
def main():
    ...
    with tf.name_scope('Loss'):
        Digit_Softmax_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=Predicted_Digit_logits,
                                                                                    labels=ph_y,
    ...
    optimizer = tf.train.AdamOptimizer(learning_rate=lear_rate,
                                       beta1=bta1,
                                       beta2=bta2,
                                       epsilon=epsln).minimize(Digit_Softmax_loss)
    ...
    with tf.Session() as sess:
        ...
        for itr in range(2001):
            ...
            _ = sess.run([optimizer], feed_dict={ph_x: batch[0], ph_y: batch[1], keep_prob: .5})
            if itr % 10 == 0:
                [train_acc, s] = sess.run([Classification_accuracy, summ],
                                          feed_dict={ph_x: batch[0], ph_y: batch[1], keep_prob: 1.})
                Train_writer.add_summary(s, itr)
                [test_acc, s] = sess.run([Classification_accuracy, summ],
                                         feed_dict={ph_x: mnist.test.images, ph_y: mnist.test.labels, keep_prob: 1.})
